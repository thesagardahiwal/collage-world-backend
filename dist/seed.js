...
const seedData = () => __awaiter(void 0, void 0, void 0, function* () {
    try {
        ...
        const users = yield user_1.default.find().limit(5);
        // Create Doubts with Answers
        for (let i = 1; i <= 20; i++) {
            const asker = users[Math.floor(Math.random() * users.length)]._id;
            const doubt = yield doubt_1.default.create({
                ...
                votes: Math.floor(Math.random() * 50 - 25), // Random upvotes/downvotes
            });
            const answers = [];
            for (let j = 1; j <= 3; j++) {
